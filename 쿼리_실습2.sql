...
# 해당라인이 나갔다고 해당 라인만 보지말고 전라인 까지를 확실히 봐라
...
#실습 2-14
SELECT * FROM `bank_transaction` WHERE `t_a_no` = 2   ORDER BY `t_amount` DESC;
...
FROM `bank_transaction`;
# t_dist가 1일때 카운트를 1로 잡아(THEN 1)서 늘린다.
...
#8버전에서 group by 시에 같은 테이블의 값만 select가 가능하다.
...
#8버전에서 group by 시에 같은 테이블의 값만 select가 가능하다.
#Error Code: 1055. Expression #1 of SELECT list is not in GROUP BY clause and contains nonaggregated column 에러
